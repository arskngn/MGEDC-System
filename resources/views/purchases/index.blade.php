<x-app-layout>
    <div
        x-data="{
            ...
            openPayment(p) {
                this.paymentPurchase = p;
                this.paymentOpen = true;
                this.openMoreId = null;
                this.$nextTick(() => {
                    const inp = this.$el.querySelector('.payment-paying-input');
                    if (inp) { inp.max = p.due; inp.value = ''; }
                });
            },
        }"
    ...
    >
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <h2 class="text-xl font-bold text-[#0a1233] shrink-0">All Purchases</h2>

            <div class="flex flex-wrap items-center gap-2 lg:gap-2 lg:ml-auto lg:justify-end">
            <form
                method="GET"
                action="{{ route('purchases.all') }}"
            ...
            >
                ...
                {{-- Date range: input + search + preset / dual calendar panel (same width as text search) --}}
                @include('components.date-range-picker')

                @if(request()->hasAny(['search', 'date_from', 'date_to']))
                    <a href="{{ route('purchases.all') }}" class="text-sm text-gray-500 hover:text-[#5542ff] px-2">Clear</a>
                @endif
            </form>

            <div class="flex flex-wrap items-center gap-2 shrink-0">
                @if(auth()->user()->hasPermission('New Purchase'))
                    <a href="{{ route('purchases.create') }}" class="inline-flex items-center px-4 py-2 bg-white border border-[#5542ff] text-[#5542ff] rounded-md hover:bg-[#5542ff]/5 transition-colors text-sm font-semibold">
                ...
                @if(auth()->user()->hasPermission('Download Purchase PDF') || auth()->user()->hasPermission('Download Purchase CSV'))
                    <div class="relative">
                        ...
                        >
                            @if(auth()->user()->hasPermission('Download Purchase PDF'))
                                <a href="{{ route('purchases.export.pdf', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                            ...
                            @endif
                            @if(auth()->user()->hasPermission('Download Purchase CSV'))
                                <a href="{{ route('purchases.export.csv', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                    <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                    Download CSV
                                </a>
                            @endif
                ...
                <table class="w-full text-left border-collapse min-w-[1100px]">
                    <thead>
                        <tr class="bg-[#5542ff] text-white">
                            ...
                            <th class="px-4 py-3.5 text-[11px] font-bold uppercase tracking-wide">Total Amount | Warehouse</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($purchases as $purchase)
                            @php
                                ...
                                $canReturnNav = auth()->user()->hasPermission('All Purchase Return') || auth()->user()->hasPermission('Item Of Purchase Return');
                            @endphp
                            <tr class="hover:bg-gray-50/90 transition-colors bg-white">
                                <td class="px-4 py-3.5 align-top">
                                    <div class="flex items-start gap-1.5">
                                        ...
                                        @endif
                                        <div>
                                            <div class="text-sm font-bold text-gray-900">{{ $purchase->invoice_no }}</div>
                                            <div class="text-xs text-gray-500 mt-0.5">{{ $purchase->purchase_date->format('d M, Y') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm font-semibold text-[#5542ff]">{{ $purchase->supplier->name }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $phoneDisp }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm font-bold text-gray-900">{{ formatCurrency($purchase->subtotal) }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $purchase->warehouse->name }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-500">{{ formatCurrency($purchase->discount) }}</div>
                                    <div class="text-sm font-bold text-gray-900 mt-0.5">{{ formatCurrency($purchase->payable_amount) }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-500">{{ formatCurrency($purchase->paid_amount) }}</div>
                                    <div class="text-sm font-bold text-gray-900 mt-0.5">{{ formatCurrency($purchase->due_amount) }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top text-right">
                                    <div class="flex items-center justify-end gap-2 flex-nowrap whitespace-nowrap">
                                        @if(auth()->user()->hasPermission('Edit Purchase'))
                                            <a href="{{ route('purchases.edit', $purchase) }}" class="inline-flex items-center px-2.5 py-1.5 bg-white border border-[#5542ff] text-[#5542ff] rounded-md hover:bg-[#5542ff]/5 text-[11px] font-bold">
                                        ...
                                        <div class="inline-flex">
                                            <button
                                                type="button"
                                                @click.stop="toggleMore({{ $purchase->id }}, $event)"
                                            ...
                                            <template x-teleport="body">
                                                ...
                                                >
                                                    @if(auth()->user()->hasPermission('Store Supplier Payment') && $purchase->due_amount > 0.009)
                                                        <button
                                                            type="button"
                                                            @click="openPayment({ id: {{ $purchase->id }}, invoice: @js($purchase->invoice_no), supplier: @js($purchase->supplier->name), payable: @js(number_format((float)$purchase->payable_amount, 2, '.', '')), due: @js(number_format($purchase->due_amount, 2, '.', '')) })"
                                                    ...
                                                    @endif
                                                    @if(auth()->user()->hasPermission('Download Purchase Invoice PDF'))
                                                        <a href="{{ route('purchases.invoice.pdf', $purchase) }}" class="flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                            <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                                            Download details
                                                        </a>
                                                    @endif
                                                    @if($canReturnNav && $purchase->purchase_returns_count > 0)
                                                        <a href="{{ route('purchases.return', ['purchase_id' => $purchase->id]) }}" class="flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                            <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                                                            View return details
                                                        </a>
                                                    @endif
                                                </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-500">No purchases found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="paymentOpen" x-cloak class="fixed inset-0 z-[210] overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 z-10" @click.stop>
                    ...
                    <div x-show="paymentPurchase" x-cloak>
                        <form method="POST" class="space-y-4" x-bind:action="'{{ url('/purchases') }}/' + paymentPurchase.id + '/payment'">
                            @csrf
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 mb-1">Invoice No.</label>
                                <input type="text" :value="paymentPurchase.invoice" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm text-gray-700" />
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 mb-1">Supplier</label>
                                <input type="text" :value="paymentPurchase.supplier" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm text-gray-700" />
                            </div>
                            <div>
                                ...
                                <div class="flex rounded-lg border border-gray-200 overflow-hidden">
                                    ...
                                    <input type="text" :value="paymentPurchase.payable" disabled class="flex-1 px-3 py-2 bg-gray-100 text-sm text-gray-700" />
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="flex rounded-lg border border-gray-200 overflow-hidden">
                                    <span class="px-3 py-2 bg-gray-100 text-gray-500 text-sm">{{ currencySymbol() }}</span>
                                    <input type="number" name="paying_amount" step="0.01" min="0.01" required class="flex-1 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#5542ff] payment-paying-input" placeholder="0.00" />
                                </div>
                                @error('paying_amount')
