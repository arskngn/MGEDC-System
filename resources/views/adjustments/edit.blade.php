<x-app-layout>
    @php
        $initialItems = $adjustment->items->map(fn ($i) => [
    ...
    @endphp
    <script type="application/json" id="adjustment-initial-items">@json($initialItems)</script>
    <div
        ...
        x-data="{
            ...
            items: [],
            init() {
                const el = document.getElementById('adjustment-initial-items');
            ...
            async searchProducts() {
                ...
                this.searchTimer = setTimeout(async () => {
                    try {
                        const r = await fetch('{{ route('adjustments.products.search') }}?q=' + encodeURIComponent(q), { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } });
    ...
    >
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-bold text-[#0a1233]">Edit Adjustment</h2>
        ...
        <form action="{{ route('adjustments.update', $adjustment) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tracking No. <span class="text-red-500">*</span></label>
                    <input type="text" name="tracking_no" value="{{ old('tracking_no', $adjustment->tracking_no) }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm bg-gray-100" readonly />
                ...
                <div>
                    ...
                    <select name="warehouse_id" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-1 focus:ring-[#4634ff]">
                        ...
                        @foreach($warehouses as $w)
                            <option value="{{ $w->id }}" @selected(old('warehouse_id', $adjustment->warehouse_id) == $w->id)>{{ $w->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="adjustment_date" value="{{ old('adjustment_date', $adjustment->adjustment_date->format('Y-m-d')) }}" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-1 focus:ring-[#4634ff]" />
            ...
            <div>
                ...
                <textarea name="note" rows="3" placeholder="Add any notes about this adjustment..." class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-[#4634ff]">{{ old('note', $adjustment->note) }}</textarea>
