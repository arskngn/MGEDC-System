<x-app-layout>
    ...
    >
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <h2 class="text-xl font-bold text-[#0a1233] shrink-0">All Adjustments</h2>
            ...
            >
                ...
                @if(request()->hasAny(['search', 'date_from', 'date_to']))
                    <a href="{{ route('adjustments.index') }}" class="text-sm text-gray-500 hover:text-[#5542ff] px-2">Clear</a>
                @endif
            </form>

            <div class="flex flex-wrap items-center gap-2 shrink-0">
                @if(auth()->user()->hasPermission('Create Adjustment'))
                    <a href="{{ route('adjustments.create') }}" class="inline-flex items-center px-4 py-2 bg-white border border-[#5542ff] text-[#5542ff] rounded-md hover:bg-[#5542ff]/5 transition-colors text-sm font-semibold">
                ...
                @if(auth()->user()->hasPermission('Download Adjustment PDF') || auth()->user()->hasPermission('Download Adjustment CSV'))
                    <div class="relative">
                        ...
                        >
                            @if(auth()->user()->hasPermission('Download Adjustment PDF'))
                                <a href="{{ route('adjustments.export.pdf', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                            ...
                            @endif
                            @if(auth()->user()->hasPermission('Download Adjustment CSV'))
                                <a href="{{ route('adjustments.export.csv', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                    <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                    Download CSV
                                </a>
                            @endif
                ...
                <table class="w-full text-left border-collapse min-w-[1100px]">
                    <thead>
                        <tr class="bg-[#5542ff] text-white">
                            ...
                            <th class="px-4 py-3.5 text-[11px] font-bold uppercase tracking-wide">Warehouse</th>
                        ...
                        @forelse($adjustments as $i => $adjustment)
                            <tr class="hover:bg-gray-50/90 transition-colors bg-white">
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm font-semibold text-gray-900">{{ ($adjustments->currentPage() - 1) * $adjustments->perPage() + $i + 1 }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm font-bold text-gray-900">{{ $adjustment->tracking_no }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-600">{{ $adjustment->adjustment_date->format('d M, Y') }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-600">{{ $adjustment->warehouse->name }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-600">{{ count($adjustment->items) }}</div>
                                </td>
                                <td class="px-4 py-3.5 text-right align-top">
                                    <div class="flex justify-end gap-2 flex-wrap">
                                        @if(auth()->user()->hasPermission('Edit Adjustment'))
                                            <a href="{{ route('adjustments.edit', $adjustment) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-600 border border-blue-300 rounded hover:bg-blue-100 transition-colors text-xs font-semibold">
                                        ...
                                        @endif
                                        @if(auth()->user()->hasPermission('Download Adjustment PDF'))
                                            <a href="{{ route('adjustments.pdf', $adjustment) }}" class="inline-flex items-center px-3 py-1.5 bg-green-50 text-green-600 border border-green-300 rounded hover:bg-green-100 transition-colors text-xs font-semibold">
