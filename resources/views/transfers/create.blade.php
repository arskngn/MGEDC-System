<x-app-layout>
    <div
        ...
        x-data="{
            ...
            searchLoading: false,
            searchTimer: null,
            warehouseId: '{{ old('from_warehouse_id') }}',
            items: [],
    ...
    >
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-bold text-[#0a1233]">New Transfer</h2>
        ...
        <form action="{{ route('transfers.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Date <span class="text-red-500">*</span></label>
                    <input type="date" name="transfer_date" value="{{ old('transfer_date', now()->format('Y-m-d')) }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-1 focus:ring-[#4634ff]" />
            ...
            <div>
                ...
                <textarea name="note" rows="3" placeholder="Add any notes about this transfer..." class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-[#4634ff]">{{ old('note') }}</textarea>
