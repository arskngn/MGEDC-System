<x-app-layout>
    @php
        $initialItems = $sale->items->map(fn ($i) => [
    ...
    @endphp
    <script type="application/json" id="sale-initial-items">@json($initialItems)</script>
    <div
        ...
        x-data="{
            ...
            searchTimer: null,
            items: [],
            discount: {{ old('discount', $sale->discount) }},
            hasReturns: @json($hasReturns),
            init() {
                const el = document.getElementById('sale-initial-items');
    ...
    >
        <div class="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
            <h2 class="text-xl font-bold text-[#0a1233]">Edit Sale</h2>
            <div class="flex flex-wrap items-center gap-2">
                @if(auth()->user()->hasPermission('Store Sale Return'))
                    <a href="{{ route('sales.return.create', $sale) }}" class="inline-flex items-center px-4 py-2 bg-white border border-[#5542ff] text-[#5542ff] rounded-lg text-sm font-semibold hover:bg-[#5542ff]/5">
                        Sale return
                    </a>
                @endif
        ...
        @if($hasReturns)
            <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 flex gap-3 items-start">
                ...
                <div class="text-sm">
                    <p class="font-bold text-red-800">Some products have been returned from this sale</p>
                    <p class="text-red-700 mt-1">You cannot edit a sale after any quantity has been returned.</p>
                </div>
            </div>
        @endif

        <form action="{{ route('sales.update', $sale) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Invoice No. <span class="text-red-500">*</span></label>
                    <input type="text" name="invoice_no" value="{{ old('invoice_no', $sale->invoice_no) }}" :disabled="hasReturns" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm disabled:bg-gray-100" />
                    @error('invoice_no')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Customer <span class="text-red-500">*</span></label>
                    <select name="customer_id" :disabled="hasReturns" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm disabled:bg-gray-100">
                        @foreach($customers as $c)
                            <option value="{{ $c->id }}" @selected(old('customer_id', $sale->customer_id) == $c->id)>{{ $c->name }} {{ $c->phone ? '+'.$c->phone : '' }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="sale_date" value="{{ old('sale_date', $sale->sale_date->format('Y-m-d')) }}" :disabled="hasReturns" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm disabled:bg-gray-100" />
                ...
                <div>
                    ...
                    <select name="warehouse_id" :disabled="hasReturns" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm disabled:bg-gray-100">
                        @foreach($warehouses as $w)
                            <option value="{{ $w->id }}" @selected(old('warehouse_id', $sale->warehouse_id) == $w->id)>{{ $w->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="relative" @click.outside="searchResults = []">
                ...
                <div class="flex rounded-lg border border-gray-200 overflow-hidden" :class="hasReturns ? 'opacity-50 pointer-events-none' : ''">
                    ...
                    <input type="text" x-model="productQuery" @input="searchProducts()" autocomplete="off" placeholder="Product Name or SKU" class="flex-1 px-3 py-2 text-sm focus:outline-none" />
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="note" rows="5" :disabled="hasReturns" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm disabled:bg-gray-100">{{ old('note', $sale->note) }}</textarea>
                </div>
                <div class="space-y-3">
                    <div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
                            <span class="px-3 py-2 bg-gray-100 text-gray-500">{{ currencySymbol() }}</span>
                            <input type="text" readonly value="{{ number_format((float) $sale->paid_amount, 2, '.', '') }}" class="flex-1 px-3 py-2 bg-gray-50 text-sm" />
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="flex rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
                            <span class="px-3 py-2 bg-gray-100 text-gray-500">{{ currencySymbol() }}</span>
                            <input type="text" readonly value="{{ number_format($sale->due_amount, 2, '.', '') }}" class="flex-1 px-3 py-2 bg-gray-50 text-sm font-semibold" />
                        </div>
                    </div>
                </div>
            </div>
