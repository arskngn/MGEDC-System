...
<head>
    <meta charset="utf-8">
    <title>Sale {{ $sale->invoice_no }}</title>
    <style>
...
<body>
    <table class="header-table">
        <tr>
            ...
            <td class="brand">
                ...
                <div class="meta" style="margin-top:8px;"><strong>Invoice No.:</strong> #{{ $sale->invoice_no }}</div>
                <div class="meta"><strong>Date:</strong> {{ $sale->sale_date->format('d F Y') }}</div>
                <div class="meta"><strong>Warehouse:</strong> {{ $sale->warehouse->name }}</div>
            </td>
        </tr>
    </table>
    ...
    <div class="customer-block">
        ...
        <div><strong>Mobile:</strong> {{ $sale->customer->phone ?? '—' }}</div>
    ...
    <div class="summary">
        <table>
            ...
            <tr>
                <td>Grand Total</td>
                <td>{{ formatCurrency($sale->receivable_amount) }}</td>
            </tr>
            <tr>
                <td>Received</td>
    ...
    @if($sale->note)
        <p style="margin-top:20px;font-size:10px;color:#4b5563;"><strong>Note:</strong> {{ $sale->note }}</p>
    @endif
