<x-app-layout>
        ...
        x-data="{
            ...
            openPayment(s) {
                ...
                this.$nextTick(() => {
                    ...
                    if (inp) { inp.max = s.due; inp.value = ''; }
                });
            },
        }"
    ...
    >
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <h2 class="text-xl font-bold text-[#0a1233] shrink-0">All Sales</h2>

            <div class="flex flex-wrap items-center gap-2 lg:gap-2 lg:ml-auto lg:justify-end">
            <form
                method="GET"
                action="{{ route('sales.all') }}"
            ...
            >
                ...
                @if(request()->hasAny(['search', 'date_from', 'date_to']))
                    <a href="{{ route('sales.all') }}" class="text-sm text-gray-500 hover:text-[#5542ff] px-2">Clear</a>
                @endif
            </form>

            <div class="flex flex-wrap items-center gap-2 shrink-0">
                @if(auth()->user()->hasPermission('Create Sale'))
                    <a href="{{ route('sales.create') }}" class="inline-flex items-center px-4 py-2 bg-white border border-[#5542ff] text-[#5542ff] rounded-md hover:bg-[#5542ff]/5 transition-colors text-sm font-semibold">
                ...
                @if(auth()->user()->hasPermission('Download Sales PDF') || auth()->user()->hasPermission('Download Sales CSV'))
                    <div class="relative">
                        ...
                        >
                            @if(auth()->user()->hasPermission('Download Sales PDF'))
                                <a href="{{ route('sales.export.pdf', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                            ...
                            @endif
                            @if(auth()->user()->hasPermission('Download Sales CSV'))
                                <a href="{{ route('sales.export.csv', request()->query()) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                    <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                    Download CSV
                                </a>
                            @endif
                ...
                <table class="w-full text-left border-collapse min-w-[1100px]">
                    <thead>
                        <tr class="bg-[#5542ff] text-white">
                            ...
                            <th class="px-4 py-3.5 text-[11px] font-bold uppercase tracking-wide">Discount | Receivable</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($sales as $sale)
                            @php
                                ...
                                $hasReturn = $sale->sale_returns_count > 0;
                            @endphp
                            <tr class="hover:bg-gray-50/90 transition-colors bg-white">
                                <td class="px-4 py-3.5 align-top">
                                    <div class="flex items-start gap-1.5">
                                        @if($hasReturn)
                                            <span class="inline-block w-2 h-2 rounded-full bg-red-500 mt-1.5 shrink-0 cursor-pointer" title="Has return(s)"></span>
                                        @endif
                                        <div>
                                            <div class="text-sm font-bold text-gray-900">{{ $sale->invoice_no }}</div>
                                            <div class="text-xs text-gray-500 mt-0.5">{{ $sale->sale_date->format('d M, Y') }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm font-semibold text-[#5542ff]">{{ $sale->customer->name }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $phoneDisp }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-500">{{ $sale->warehouse->name }}</div>
                                    <div class="text-sm font-bold text-gray-900 mt-0.5">{{ formatCurrency($sale->subtotal) }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-500">{{ formatCurrency($sale->discount) }}</div>
                                    <div class="text-sm font-bold text-gray-900 mt-0.5">{{ formatCurrency($sale->receivable_amount) }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top">
                                    <div class="text-sm text-gray-500">{{ formatCurrency($sale->paid_amount) }}</div>
                                    <div class="text-sm font-bold text-gray-900 mt-0.5">{{ formatCurrency($sale->due_amount) }}</div>
                                </td>
                                <td class="px-4 py-3.5 align-top text-right">
                                    <div class="flex items-center justify-end gap-2 flex-nowrap whitespace-nowrap">
                                        @if(auth()->user()->hasPermission('Edit Sale'))
                                            <a href="{{ route('sales.edit', $sale) }}" class="inline-flex items-center px-2.5 py-1.5 bg-white border border-[#5542ff] text-[#5542ff] rounded-md hover:bg-[#5542ff]/5 text-[11px] font-bold">
                                        ...
                                        <div class="inline-flex">
                                            <button
                                                type="button"
                                                @click.stop="toggleMore({{ $sale->id }}, $event)"
                                            ...
                                            <template x-teleport="body">
                                                ...
                                                >
                                                    @if(auth()->user()->hasPermission('Store Customer Payment') && $sale->due_amount > 0.009)
                                                        <button
                                                            type="button"
                                                            @click="openPayment({ id: {{ $sale->id }}, invoice: @js($sale->invoice_no), customer: @js($sale->customer->name), receivable: @js(number_format((float)$sale->receivable_amount, 2, '.', '')), due: @js(number_format($sale->due_amount, 2, '.', '')) })"
                                                    ...
                                                    @endif
                                                    @if(auth()->user()->hasPermission('Store Sale Return'))
                                                        <a href="{{ route('sales.return.create', $sale) }}" class="flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                    ...
                                                    @endif
                                                    @if(auth()->user()->hasPermission('Download Sale Invoice PDF'))
                                                        <a href="{{ route('sales.invoice.pdf', $sale) }}" class="flex items-center px-3 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                            <svg class="h-4 w-4 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                                            Download Invoice
                                                        </a>
                                                    @endif
        ...
        <div x-show="paymentOpen" x-cloak class="fixed inset-0 z-[210] overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6 z-10" @click.stop>
                    ...
                    <div x-show="paymentSale" x-cloak>
                        <form method="POST" class="space-y-4" x-bind:action="'{{ url('/sales') }}/' + paymentSale.id + '/payment'">
                            @csrf
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 mb-1">Invoice No.</label>
                                <input type="text" :value="paymentSale.invoice" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm text-gray-700" />
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-500 mb-1">Customer</label>
                                <input type="text" :value="paymentSale.customer" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm text-gray-700" />
                            </div>
                            <div>
                                ...
                                <div class="flex rounded-lg border border-gray-200 overflow-hidden">
                                    ...
                                    <input type="text" :value="paymentSale.receivable" disabled class="flex-1 px-3 py-2 bg-gray-100 text-sm text-gray-700" />
                                </div>
                            </div>
                            <div>
