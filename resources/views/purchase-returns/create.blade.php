<x-app-layout>
    @php
        $initialLines = collect($lines)
            ->map(function (array $l): array {
                return array_merge($l, [
                    'max_return' => (float) $l['stock_quantity'],
                ]);
            })
            ->values()
            ->all();
    @endphp
    <script type="application/json" id="purchase-return-initial-lines">@json($initialLines)</script>
    <div
        ...
        x-data="{
            ...
            init() {
                const el = document.getElementById('purchase-return-initial-lines');
    ...
    >
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-bold text-[#0a1233]">Add Purchase Return</h2>
        ...
        <form action="{{ route('purchases.return.store', $purchase) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Invoice No. <span class="text-red-500">*</span></label>
                    <input type="text" value="{{ $invoiceNo }}" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Supplier <span class="text-red-500">*</span></label>
                    <input type="text" value="{{ $purchase->supplier->name }}" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Date</label>
                    <input type="date" name="return_date" value="{{ old('return_date', now()->format('Y-m-d')) }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-1 focus:ring-[#5542ff]" />
                ...
                <div>
                    ...
                    <input type="text" value="{{ $purchase->warehouse->name }}" disabled class="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2 text-sm" />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="note" rows="5" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:ring-1 focus:ring-[#5542ff]" placeholder="Optional note">{{ old('note') }}</textarea>
                </div>
                <div class="space-y-3">
                    <div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg border border-gray-200 bg-gray-100 overflow-hidden">
                            <span class="px-3 py-2 bg-gray-100 text-gray-500 text-sm">{{ currencySymbol() }}</span>
                            <input type="text" value="0.00" readonly class="flex-1 px-3 py-2 bg-gray-100 text-sm" />
                        </div>
                    </div>
                    <div>
