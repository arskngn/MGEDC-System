<x-app-layout>
    ...
    >
        <div class="flex items-center justify-between gap-4">
            <h2 class="text-xl font-bold text-[#0a1233]">Unsettled Payments with {{ $customer->name }}</h2>

            <a href="{{ route('customers.index') }}" class="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-semibold inline-flex items-center">
        ...
        @php
            $canClear = (float) $receivableTotal > 0.009 || (float) $payableTotal > 0.009;
        @endphp
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                @if(auth()->user()->hasPermission('Clear Payment Of Customer'))
                    <button
                        type="button"
                        class="inline-flex items-center px-4 py-2 rounded-lg bg-[#2563eb] border border-[#2563eb] text-white font-semibold hover:bg-[#1d4ed8] transition-colors shadow-sm disabled:opacity-50 disabled:pointer-events-none {{ $canClear ? '' : 'opacity-50 cursor-not-allowed pointer-events-none' }}"
                ...
                <table class="w-full text-left border-collapse min-w-[720px]">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($lines as $i => $line)
                            @php $isReceivable = $line['reason'] === 'Sale'; @endphp
                            <tr class="hover:bg-gray-50/90 transition-colors bg-white">
                                ...
                                <td class="px-4 py-4 align-middle text-sm font-bold text-right {{ $isReceivable ? 'text-green-600' : 'text-red-600' }}">
                                    {{ formatCurrency($line['amount']) }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-gray-500">No unsettled payments for this customer.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 py-8">
                <div class="relative bg-white rounded-xl shadow-xl max-w-md w-full z-10 border border-gray-100" @click.stop>
                    <div class="flex items-start justify-between px-6 pt-5 pb-3 border-b border-gray-100">
                        <h3 class="text-lg font-bold text-gray-900">Clear Full Payment with {{ $customer->name }}</h3>
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        <div class="rounded-lg border border-amber-200 bg-amber-50 text-amber-950 px-4 py-3">
                            ...
                            <div class="text-[12px] text-amber-950/90 leading-relaxed">
                                <span class="italic">If you click on “Clear Payment” button, all receivable and payable dues will be cleared. So be sure before click.</span>
                            </div>
                        </div>

                        <form method="POST" action="{{ route('customers.payments.clear', $customer) }}" class="flex gap-3">
                            @csrf
                            <button
                                type="button"
