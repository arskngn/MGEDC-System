<x-app-layout>
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-4">
            <h2 class="text-xl font-bold text-[#0a1233]">Send Notification to {{ $customer->name }}</h2>
            <a href="{{ route('customers.notifications.single', $customer) }}"
        ...
        @php $customerEmail = filled($customer->email); $customerPhone = filled($customer->phone); $defaultChannel = $customerEmail ? 'email' : ($customerPhone ? 'sms' : 'email'); @endphp
        ...
        <form method="POST" action="{{ route('customers.notifications.single.send', $customer) }}" class="bg-gray-50 p-8 rounded-xl border border-gray-200 space-y-8" x-data="{ channel: '{{ $defaultChannel }}' }" x-bind:data-notif-channel="channel">
            @csrf
            ...
            <div class="space-y-6">
                <div x-show="channel === 'email'" x-cloak>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Subject <span class="text-red-500" x-show="channel === 'email'">*</span>
                    </label>
                    <input type="text"
                           name="subject"
