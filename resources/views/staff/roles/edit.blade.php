<x-app-layout>
    <div class="mb-6">
        <h2 class="text-xl font-bold text-[#0a1233]">Edit Role: {{ $role->name }}</h2>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
            <div class="p-6">
                <div class="mb-4">
                    ...
                    <input type="text" name="name" value="{{ $role->name }}" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm" placeholder="Role Name">
                </div>
            </div>
        </div>
                                    ...
                                    @foreach($permissions as $permission)
                                        <label class="flex items-center space-x-3 cursor-pointer group">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" 
                                                ...
                                                class="h-4 w-4 text-[#4634ff] border-gray-300 rounded focus:ring-[#4634ff] transition-colors">
